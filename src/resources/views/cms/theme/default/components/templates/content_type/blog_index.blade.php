    ...
    <div class="py-16 lg:py-20">
        ...
        @foreach($blogs->items() as $index => $blog)
          ...
          <div 
            ...
            @endforeach
            <a href="{{ $blog->getUrl()}}" class="block font-sans text-lg font-semibold text-primary transition-colors hover:text-green dark:text-white dark:hover:text-secondary">
                {{ $blog->title }}
            </a>
            ...
            <div class="flex pt-8 lg:pt-16">
                @if (! $blogs->onFirstPage())
                    <a href="{{ str($currUrl)->finish('?')->finish($requestQuery->push('page='.$currPage - 1)->implode('&')) }}">
                        <span class="group ml-3 flex cursor-pointer items-center border-2 border-primary px-3 py-1 font-sans font-medium text-primary transition-colors hover:border-secondary hover:text-secondary dark:border-green-light dark:text-white dark:hover:border-secondary dark:hover:text-secondary">
                            Previous
                ...
                @foreach ($blogs->render()->offsetGet('elements') as $element)
                    ...
                    @if (is_array($element))
                        @foreach ($element as $page => $url)
                            <a href="{{ $url }}">
                                <span class="ml-3 flex cursor-pointer items-center border-2 border-primary px-3 py-1 font-sans font-medium text-primary transition-colors hover:border-secondary hover:text-secondary dark:border-green-light dark:text-white dark:hover:border-secondary dark:hover:text-secondary">
                                    {{ $page }}
                                </span>
                            </a>
                        @endforeach
